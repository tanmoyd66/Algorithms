 ...
 * 
 * Reference: Flajolet, Philippe, and G. Nigel Martin. "Probabilistic counting
...
public class HyperLogLog {
	...
	public HyperLogLog(int bitmapSize, int numHashGroups, int numHashFunctionsInEachGroup) {
	...
	private HashFunction generateUniqueHashFunction(Map<Integer, Collection<Integer>> mnMap) {
		...
		// System.out.println("Generating hashFunction with (m=" + m + ", n=" +
		// n + ")");
	...
	public static void main(String[] args) {
		...
		HyperLogLog hll = new HyperLogLog(64, 32, 8);
		for (String word : words) {
			hll.offer(word);
			test.add(word);
		}
		
		System.out.printf("Count: %d, HLL: %d\n", test.size(), hll.cardinality());
	}
